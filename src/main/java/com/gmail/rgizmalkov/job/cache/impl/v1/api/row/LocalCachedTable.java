...
import com.gmail.rgizmalkov.job.cache.api.row.CachedTable;
...
import com.gmail.rgizmalkov.job.cache.impl.v1.errors.NotDictionaryClassRuntimeException;
...
import static com.gmail.rgizmalkov.job.cache.impl.v1.api.row.LocalCachedTable.SearchMethod.FIND;
import static com.gmail.rgizmalkov.job.cache.impl.v1.api.row.LocalCachedTable.SearchMethod.LIKE;
...
public class LocalCachedTable<Type> implements CachedTable<Type> {
    ...
    private LocalCachedTable(Class<Type> type, Set<String> indexes) {
        this.type = type;
        checkTable(type);
    ...
    public static <Type> LocalCachedTable<Type> of(Class<Type> type, String ... indexes) {
        Set<String> indexes_set = new HashSet<>();
        if (indexes != null) {
            indexes_set = ImmutableSet.copyOf(indexes);
        }
        return new LocalCachedTable<Type>(type, indexes_set);
    }
